...
from impacket.dcerpc.v5.ndr import NDRCONSTRUCTEDTYPE, NDRArray, NDRLONG, NDRSMALL, NDRUniFixedArray
from impacket.dcerpc.v5.dtypes import NULL, DWORD_ARRAY, PRPC_SID, RPC_SID, ULONG, RPC_SID_IDENTIFIER_AUTHORITY
from impacket.dcerpc.v5.samr import SID_NAME_USE, SAMPR_PSID_ARRAY_OUT, SAMPR_SID_INFORMATION_ARRAY, SAMPR_SID_INFORMATION, PSAMPR_SID_INFORMATION, PSAMPR_SID_INFORMATION_ARRAY, SAMPR_PSID_ARRAY_OUT, SamrGetMembersInAlias, SamrGetMembersInAliasResponse, SamrOpenAlias, SamrOpenAliasResponse, ULONG_ARRAY, PULONG_ARRAY, SamrQueryInformationDomain, SamrQueryInformationDomainResponse, SamrEnumerateUsersInDomain, SamrEnumerateUsersInDomainResponse, SamrLookupDomainInSamServer, SamrLookupDomainInSamServerResponse, SAMPR_RID_ENUMERATION, SAMPR_ULONG_ARRAY, RPC_UNICODE_STRING, SamrCloseHandle, SamrCloseHandleResponse, SAMPR_HANDLE, PSAMPR_ENUMERATION_BUFFER, SamrEnumerateDomainsInSamServer, SamrEnumerateDomainsInSamServerResponse, SamrLookupNamesInDomain, SamrLookupNamesInDomainResponse, SamrOpenDomain, SamrOpenDomainResponse, SamrConnect5, SamrConnect5Response, SamrGetMembersInGroup, SamrGetMembersInGroupResponse, SamrConnect, SamrConnectResponse
from impacket.dcerpc.v5.srvs import NetrSessionEnum, NetrSessionEnumResponse, SESSION_INFO_10, NetrShareEnum, NetrShareEnumResponse, SHARE_INFO_1, NetrServerGetInfo, NetrServerGetInfoResponse, NetrShareGetInfo, NetrShareGetInfoResponse
...
from impacket.dcerpc.v5.lsad import LSAPR_TRUST_INFORMATION_ARRAY, LsarOpenPolicy2, LsarOpenPolicy2Response, LSAPR_HANDLE, LsarClose, LsarCloseResponse, PLSAPR_TRUST_INFORMATION_ARRAY, LSAPR_TRUST_INFORMATION
from impacket.dcerpc.v5.lsat import LSAPR_TRANSLATED_NAME_EX, LSAPR_TRANSLATED_NAME_EX_ARRAY, PLSAPR_TRANSLATED_NAME_EX_ARRAY, LSAPR_TRANSLATED_NAMES_EX, LsarLookupSids2, LsarLookupSids2Response, PLSAPR_REFERENCED_DOMAIN_LIST, LSAPR_REFERENCED_DOMAIN_LIST
import json

class lsaRPCServer(DCERPCServer):
    def __init__(self):
        DCERPCServer.__init__(self)
        self.lsarpcsvcCallBacks = {
            0: self.LsarClose,
            44: self.LsarOpenPolicy2,
            57: self.LsarLookupSids2,

        }
        self.addCallbacks(('12345778-1234-abcd-ef00-0123456789ab', '0.0'),'\\PIPE\\lsarpc', self.lsarpcsvcCallBacks)
    
    def LsarOpenPolicy2(self, data):
        answer = LsarOpenPolicy2Response()
        # Bogus Handle. Should probably do this differently.
        policyhandle = LSAPR_HANDLE(b'0x232232323232323232',b'')
        answer['PolicyHandle'] = policyhandle
        answer['ErrorCode'] = 0 
        self.log("LsarOpenPolicy2")
        return answer

    def LsarLookupSids2(self, data):
        
        cwd = os.getcwd()
        with open(cwd + "/samr_data.json") as samr_data:
            data = json.load(samr_data)
            entries = len(data['samrdata'])

            # Return ReferencedDomains
            answer = LsarLookupSids2Response()
            answer['ReferencedDomains']['Entries'] = entries
            sid1 = RPC_SID()
            SID = data['samrdata'][0]['sid']
            sid1.fromCanonical(SID)
            lsapr_trust_information_array = LSAPR_TRUST_INFORMATION_ARRAY()
            lsapr_trust_information = LSAPR_TRUST_INFORMATION()
            lsapr_trust_information['Name'] = data['samrdata'][0]['domainname']
            lsapr_trust_information['Sid'] = sid1
            lsapr_trust_information_array['Data'].append(lsapr_trust_information)
            plsapr_trust_information_array = PLSAPR_TRUST_INFORMATION_ARRAY()
            plsapr_trust_information_array['Data'] = lsapr_trust_information_array
            lsapr_referenced_domain_list = LSAPR_REFERENCED_DOMAIN_LIST()
            lsapr_referenced_domain_list['Entries'] = 1
            lsapr_referenced_domain_list['Domains'] = plsapr_trust_information_array
            plsapr_referenced_domain_list = PLSAPR_REFERENCED_DOMAIN_LIST()
            plsapr_referenced_domain_list['Data'] = lsapr_referenced_domain_list
            answer['ReferencedDomains'] = plsapr_referenced_domain_list
            # End

            # Return Translated Names
            snu = SID_NAME_USE()
            snu1 = snu.enumItems['SidTypeUser']
            lsapr_translated_name_ex_array = LSAPR_TRANSLATED_NAME_EX_ARRAY()
            lsapr_translated_name_ex = LSAPR_TRANSLATED_NAME_EX()
            lsapr_translated_name_ex['Use'] = snu1
            lsapr_translated_name_ex['Name'] = data['samrdata'][0]['username']
            #lsapr_translated_name_ex['DomainIndex'] = NULL
            lsapr_translated_name_ex['Flags'] = 0x0000001
            lsapr_translated_name_ex_array['Data'].append(lsapr_translated_name_ex)
            plsapr_translated_name_ex_array = PLSAPR_TRANSLATED_NAME_EX_ARRAY()
            plsapr_translated_name_ex_array['Data'] = lsapr_translated_name_ex_array
            lsapr_translated_names_ex = LSAPR_TRANSLATED_NAMES_EX()
            lsapr_translated_names_ex['Entries'] = 1
            lsapr_translated_names_ex['Names'] = plsapr_translated_name_ex_array
            answer['TranslatedNames'] = lsapr_translated_names_ex
            answer['MappedCount'] = 1
            answer['ErrorCode'] = 0
        self.log("LsarLookupSids2")
        return answer

    def LsarClose(self, data):
        self.log("LsarClose")
        return 

class SAMRServer(DCERPCServer):
    def __init__(self):
        DCERPCServer.__init__(self)
        self.samrsvcCallBacks = {
            0: self.SamrConnect,
            1: self.SamrCloseHandle,
            5: self.SamrLookupDomainInSamServer,
            6: self.SamrEnumerateDomainsInSamServer,
            7: self.SamrOpenDomain,
            8: self.SamrQueryInformationDomain,
            13: self.SamrEnumerateUsersInDomain,
            17: self.SamrLookupNamesInDomain,
            25: self.SamrGetMembersInGroup,
            27: self.SamrOpenAlias,
            33: self.SamrGetMembersInAlias,
            64: self.SamrConnect5,
        }
        self.addCallbacks(('12345778-1234-ABCD-EF00-0123456789AC', '1.0'),'\\PIPE\\samr', self.samrsvcCallBacks)

    def SamrConnect(self, data):
        request = SamrConnect(data)
        answer = SamrConnectResponse()
        # Bogus Handle. Should probably do this differently.
        answer['ServerHandle'] = SAMPR_HANDLE(b'0x232232323232323232',b'')
        self.log("SamrConnect")
        return answer

    def SamrConnect5(self, data):
        request = SamrConnect5(data)
        self.log("SamrConnect5")
        answer = SamrConnect5Response()
        answer['OutVersion'] = 1
        answer['OutRevisionInfo']['tag'] = 1        
        answer['OutRevisionInfo']['V1']['Revision'] = 3
        answer['OutRevisionInfo']['V1']['SupportedFeatures'] = 0
        # Bogus Handle. Should probably do this differently.
        answer['ServerHandle'] = SAMPR_HANDLE(b'0x232232323232323232',b'')
        answer['ErrorCode'] = 0
        return answer

    def SamrCloseHandle(self, data):
        answer = SamrCloseHandleResponse()
        return answer

    def SamrEnumerateDomainsInSamServer(self, data):
        request = SamrEnumerateDomainsInSamServer()
        self.log("SamrEnumerateDomainsInSamServer")
        answer = SamrEnumerateDomainsInSamServerResponse()
        answer['EnumerationContext'] = request['EnumerationContext']
        answer['Buffer']['EntriesRead'] = 1
        rids = SAMPR_RID_ENUMERATION()
        # Bogus ID. Should probably do this differently.
        rids['RelativeId'] = 500
        # Bogus Name. Should probably do this differently.
        rids['Name'] = u"name"
        answer['Buffer']['Buffer'].append(rids)
        answer['CountReturned'] = 1
        answer['ErrorCode'] = 0
        
        return answer

    def SamrOpenDomain(self, data):
        request = SamrOpenDomain(data)
        self.log("SamrOpenDomain")
        answer = SamrOpenDomainResponse()
        # Set Domain Handle. Correct?
        answer['DomainHandle'] = request['ServerHandle']
        answer['ErrorCode'] = 0
        return answer


    def SamrLookupDomainInSamServer(self, data):

        return 

    def SamrGetMembersInGroup(self, data):
        self.log("here")
        print("here")
        test = "testing"
    ...
    def SamrLookupNamesInDomain(self, data):
  
        # Return the RelativeIds field
        answer = SamrLookupNamesInDomainResponse()
        ulong = ULONG_ARRAY()
        ulongdata = ULONG()
        ulongdata['Data'] = 1337
        ulong['Data'].append(ulongdata)
        ulong['MaximumCount'] = 1
        pulong = PULONG_ARRAY()
        pulong['Data'] = ulong
        answer['RelativeIds']['Count'] = 1
        answer['RelativeIds']['Element'] = pulong
        ...
        pulong['Data'] = ulong
        answer['Use']['Count'] = 1
        answer['Use']['Element'] = pulong
        answer['ErrorCode'] = 0

        return answer

    def SamrOpenAlias(self, data):
        answer = SamrOpenAliasResponse()
        # Bogus Handle. Should probably do this differently.
        answer['AliasHandle'] = SAMPR_HANDLE(b'0x232232323232323232',b'')
        answer['ErrorCode'] = 0
        return answer

    def SamrGetMembersInAlias(self, data):
        answer = SamrGetMembersInAliasResponse()
        cwd = os.getcwd()
        # Open the fake SAMR data file. Should probably make this an argument.
        with open(cwd + "/samr_data.json") as samr_data:
            data = json.load(samr_data)
            sid1 = RPC_SID()
            SID = data['samrdata'][0]['sid']
            sid1.fromCanonical(SID)
            si = PSAMPR_SID_INFORMATION()
            si['SidPointer'] = sid1
            answer['Members']['Count'] = 0x01
            answer['Members']['Sids'].append(si)
            answer['ErrorCode'] = 0
            self.log("SamrGetMembersInAlias")
        return answer

    def SamrEnumerateUsersInDomain(self, data):
        request = SamrEnumerateUsersInDomain()
        self.log("usersindomain")
        return

    def SamrQueryInformationDomain(self, data):
        return
...
class SRVSServer(DCERPCServer):
    def __init__(self):
        ...
        self.addCallbacks(('4B324FC8-1670-01D3-1278-5A47BF6EE188', '3.0'),'\\PIPE\\srvrsvc', self.srvsvcCallBacks)
    ...
    def NetrSessionEnum(self, data):

        request = NetrSessionEnum(data)
        self.log("NetrSessionEnum")
        cwd = os.getcwd()
        # Open the fake session data file. Should probably make this an argument.
        with open(cwd + "/session_data.json") as json_data:
            data = json.load(json_data)
            entries = len(data['sessions'])
            sessionEnum = NetrSessionEnumResponse()
            sessionEnum['InfoStruct']['Level'] = 10
            sessionEnum['InfoStruct']['SessionInfo']['tag'] = 10
            sessionEnum['TotalEntries'] = entries
            sessionEnum['InfoStruct']['SessionInfo']['Level10']['EntriesRead'] = entries
            sessionEnum['ErrorCode'] = 0

            for i in range(0,entries):
                sessionInfo = SESSION_INFO_10()   
                sessionInfo['sesi10_cname'] = "\\\\"+data['sessions'][i]['computername']+'\x00' 
                sessionInfo['sesi10_username'] = data['sessions'][i]['username']+'\x00'
                sessionInfo['sesi10_time'] = data['sessions'][i]['time']
                sessionInfo['sesi10_idle_time'] = data['sessions'][i]['idle_time']
                sessionEnum['InfoStruct']['SessionInfo']['Level10']['Buffer'].append(sessionInfo)
        return sessionEnum
...
class SimpleSMBServer:
    ...
    def __init__(self, listenAddress = '0.0.0.0', listenPort=445, configFile=''):
        ...
        self.__samrServer = SAMRServer()
        self.__samrServer.daemon = True
        self.__lsaRPCServer = lsaRPCServer()
        self.__lsaRPCServer.daemon = True
        ...
        self.__server.registerNamedPipe('samr',('127.0.0.1',self.__samrServer.getListenPort()))
        self.__server.registerNamedPipe('lsarpc',('127.0.0.1',self.__lsaRPCServer.getListenPort()))
    def start(self):
        self.__srvsServer.start()
        self.__wkstServer.start()
        self.__samrServer.start()
        self.__lsaRPCServer.start()
